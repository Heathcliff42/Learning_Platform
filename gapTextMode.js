 ...
 * @returns {void}
 */
export function GapTextMode(questions) {
  let idx = [];
  let success = 0;
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    // Each question[0] contains the text with gaps marked as [___]
    const questionText = questions[idx[i]][0];
    console.log(`Question ${i + 1}:`);
    console.log(questionText);
    
    // The correct answer is in questions[idx[i]][1]
    const correctAnswer = questions[idx[i]][1];
    ...
    if (isCorrect) {
      ...
      console.log(`The correct answer was: ${styleText("green", correctAnswer)}`);
    }
