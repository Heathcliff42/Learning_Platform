 ...
 * @returns {void}
 */
export async function AIChatMode(questions) {
  let idx = [];
  let score = 0;
  ...
  console.log("Answer in your own words, and the AI will evaluate your response.");
  prompt("\nPress [Enter] to begin conversation with AI...");
  
  for (let i = 0; i < questions.length; i++) {
    console.clear();
    console.log(styleText("cyan", "AI Chat Mode - Question " + (i + 1) + " of " + questions.length));
    
    // Display the AI's question
    const question = questions[idx[i]][0];
    ...
    console.log(styleText("green", "\nAI Evaluation: ") + evaluation.status + 
                 " | Score: " + evaluation.percentage.toFixed(1) + "%");
  ...
  if (averageScore >= 90) {
    console.log(styleText("green", "Outstanding! You have excellent knowledge of this topic!"));
  } else if (averageScore >= 75) {
    console.log(styleText("green", "Great job! You have a solid understanding of this topic."));
  } else if (averageScore >= 60) {
    console.log(styleText("yellow", "Good effort! You have a decent grasp of the topic with some areas to improve."));
  } else {
    console.log(styleText("red", "Keep studying! This topic requires more review to master."));
  }
...
async function evaluateAnswerWithAI(question, userAnswer) {
  const response = await openai.createCompletion({
    ...
    prompt: `Evaluate the following answer:\n\nQuestion: ${question}\nAnswer: ${userAnswer}\n\nProvide a score out of 100, a status (Correct, Partially Correct, Incorrect), and feedback.`,
    max_tokens: 150
  });

  const evaluationText = response.data.choices[0].text.trim();
  const [status, percentage, feedback] = evaluationText.split('\n').map(line => line.split(': ')[1]);
